...
fn fork_saves_parent_and_creates_new_task_id() {
    ...
    mode.task_doc.info.active_grants.insert(crate::runtime::Capability::RunCommand, crate::runtime::ApprovalScope::Session);
    mode.push_history_line("stale transcript".to_string());
    let mut ctx = setup_ctx();

    mode.on_user_input("/fork feature work".to_string(), &mut ctx);

    assert!(temp.path().join(format!("{parent_id}.json")).exists(), "/fork must save parent state");
    assert_ne!(mode.current_task_id(), parent_id, "/fork must create a new task-id");
