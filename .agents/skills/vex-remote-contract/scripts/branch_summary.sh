#!/usr/bin/env bash
# branch_summary.sh — Print commit/files summary only (MCP PR body flow).
# Usage:
#   branch_summary.sh [-b <branch>]
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --write-pr-body|-o|--out) die "local PR body file output is disabled; use GitHub MCP to update PR body";;
    *) die "unknown arg: $1";;
  esac
done
...
echo "- PR body update mode: \`GitHub MCP only\`"
echo
