...
- ADR-031 Batch A task-state extension work is now in the current tree: stable timeline `step_id` values, explicit command-session row identity, runtime-visible `Approved` tool state, and timeline follow-mode ownership that disables auto-follow when the operator scrolls away from the latest row.
- `src/app/layout.rs` and `src/ui/draw/mod.rs` still describe the task surface with different region terms and output semantics; follow-up work should collapse those descriptions onto one adaptive timeline/transcript/composer contract.
...
- `src/ui/draw/mod.rs` still derives header fields by parsing the formatted `status_line` string; follow-up work should replace that string coupling with structured task-surface fields once the facade boundary exposes them.
