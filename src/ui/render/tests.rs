...
fn all_modals_render_without_panic() {
    ...
    for modal in [
        OverlayModal::PatchApprove { patch_preview: "diff --git a/src/app/mod.rs b/src/app/mod.rs", scroll_offset: 0 },
        OverlayModal::ToolPermission { tool_name: "exec_command", input_preview: "echo hi", auto_approve_enabled: false },
        OverlayModal::MemoryClear,
    ] {
        tui.draw(|frame| render_overlay_modal(frame, modal)).expect("render");
    }
}
...
fn history_visual_line_count_accounts_for_newlines_and_wrapping() {
    assert_eq!(history_visual_line_count(&["first".to_string(), "a\nb".to_string(), String::new()], 80), 4);
