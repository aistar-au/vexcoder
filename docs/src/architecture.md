...
| `rmcp` | MCP (Model Context Protocol) client for external tool providers | Comparable CLIs implement MCP transport directly. | vexcoder supports `[[mcp_servers]]` config for connecting to external tool providers (ADR-024 PM-01).  `rmcp` is the standard Rust MCP client crate. |
