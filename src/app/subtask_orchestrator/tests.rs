...
fn fan_out_join_creates_tasks_for_all_team_members() {
    ...
    let decomp = orc.schedule_team("parent-1", &team, &agents, "do the work").unwrap();
...
fn sequential_schedule_creates_only_first_member_task() {
    ...
    let decomp = orc.schedule_team("parent-seq", &team, &agents, "do the work").unwrap();
    assert_eq!(decomp.session_task_ids.len(), 1);
    let state = TaskState::load(orc.state_dir.as_path(), "parent-seq").unwrap();
    assert_eq!(state.session_tasks.iter().filter(|t| t.agent_id == "alpha").count(), 1);
    assert_eq!(state.session_tasks.iter().filter(|t| t.agent_id == "beta").count(), 0);
}

#[test]
fn corrupt_parent_state_propagates_error() {
    let (_dir, orc) = setup_orchestrator();
    std::fs::write(orc.state_dir.join("parent-corrupt.json"), "{ not valid json").unwrap();
    let err = orc.schedule_team("parent-corrupt", &make_team("t", &["alpha"], TeamScheduler::FanOutJoin), &[make_agent("alpha")], "work").unwrap_err();
    assert!(err.to_string().contains("Failed to deserialize state file"), "got: {err:#}");
}
