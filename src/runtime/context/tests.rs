...
fn make_ctx() -> (RuntimeContext, mpsc::UnboundedReceiver<UiUpdate>) {
    ...
    let ctx = RuntimeContext::new(
        ConversationManager::new_mock(ApiClient::new_mock(Arc::new(MockApiClient::new(vec![]))), HashMap::new()),
        tx,
...
async fn start_pulse_dispatches_stream_delta_and_completion() {
    ...
    let mut ctx = RuntimeContext::new(ConversationManager::new_mock(client, HashMap::new()), tx, CancellationToken::new());
    ctx.start_pulse("test input".to_string());
    let mut saw_delta = false;
    let mut saw_complete = false;
    loop {
        match tokio::time::timeout(Duration::from_millis(500), rx.recv()).await {
            Ok(Some(UiUpdate::StreamDelta(_))) | Ok(Some(UiUpdate::StreamBlockDelta { .. })) => saw_delta = true,
            Ok(Some(UiUpdate::PulseComplete)) => { saw_complete = true; break; }
...
fn start_pulse_without_tokio_runtime_emits_error() {
    ...
    match rx.try_recv().expect("expected error") {
        UiUpdate::Error(msg) => assert!(msg.contains("requires active Tokio runtime"), "got: {msg}"),
