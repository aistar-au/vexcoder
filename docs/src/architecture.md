...
|-------|----------|--------------------------|
| `reqwest` `multipart` feature | **Adopted now** | RFC 7578 readiness belongs on the existing HTTP client seam in `src/api/client/mod.rs`.  The CLI surface is still text-only, but enabling multipart on the existing client avoids a second transport stack when media blocks are added later. |
| `oauth2` native code exchange (`CodeTokenRequest`) | **Adopted now** | RFC 8252 native-app flow now stays localized in `src/net/oauth.rs`: loopback redirect validation, PKCE authorization URL construction, and authorization-code exchange share one seam. |
...
| HTTP/2 on the existing `reqwest` / `hyper` / `hyper-util` / `axum` stack | **Adopted now** | The current transport stack already owns HTTP negotiation, so h2 readiness is best delivered by enabling negotiated HTTP/2 and ALPN on that stack instead of introducing a second client or server layer. |
...
- ADR-034 defines the proposed second-stage multi-agent lane: worktree-isolated agent definitions, orchestrator-owned session-task lifecycle, `/agents`, `/watch`, and explicit session-task release surfaces, plus delegation-time concurrency and prompt-size enforcement built on the canonical ADR-025/ADR-030 contracts. The current hardening pass makes the delegation cap serialized, adds release-route and concurrency-stress coverage, and normalizes parent-task watch rollups onto the same lowercase status surface used by session tasks.
