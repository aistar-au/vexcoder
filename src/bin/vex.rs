...
use vexapi::app::{task_graph_rollup_path, todos_rollup_path, write_projection_rollup};
use vexapi::batch_mode::{AutoApproveScope, BatchRunOpts, OutputFormat, run_batch};
use vexapi::config::Config;
use vexapi::disk_policy::{self, DiskPolicyMode};
use vexapi::doctor::run_doctor;
use vexapi::exec::{parse_exec_command, run_exec};
use vexapi::export::{ExportFormat, render_task_export};
use vexapi::init::run_init;
use vexapi::pr_summary::{run_branch, run_pr_summary};
use vexapi::privacy::render_privacy_markdown;
use vexapi::runtime::{TaskState, TaskStatus, ToolPolicy};
use vexapi::serve_local_api;
use vexapi::startup::emit_model_endpoint_warnings;
...
) -> Result<vexapi::credentials::CredentialsAction> {
    use vexapi::credentials::CredentialsAction;
...
async fn run_print(
    ...
    let request_id = vexapi::observability::new_request_id();
    ...
    vexapi::observability::attach_standard_baggage(
        &print_span,
        "cli",
        "print",
        Some(&request_id),
...
async fn main() -> Result<ExitCode> {
    ...
    let cli = Cli::parse();
    let _telemetry_guard = vexapi::observability::init_cli_observability(
        vexapi::observability::CliTelemetryOptions {
    ...
    match cli.command {
        ...
        Some(Commands::InstallHooks) => {
            ...
            vexapi::git_hooks::install_hooks(&cwd)?;
            return Ok(ExitCode::SUCCESS);
        }
        Some(Commands::UninstallHooks) => {
            let cwd = std::env::current_dir()?;
            vexapi::git_hooks::uninstall_hooks(&cwd)?;
            return Ok(ExitCode::SUCCESS);
        }
        Some(Commands::Skills { sub }) => {
            let cwd = std::env::current_dir()?;
            let mut registry = vexapi::skills::SkillsRegistry::load(&cwd)?;
            match sub {
        ...
        Some(Commands::Credentials { sub }) => {
            use vexapi::credentials::run_credentials;
    ...
    eprintln!("[vex] interactive TUI has moved to the standalone AozCoder client.");
    eprintln!("[vex] use --print <prompt> for headless batch mode.");
    Ok(ExitCode::FAILURE)
}
