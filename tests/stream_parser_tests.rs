...
use vexcoder::runtime::RuntimeSignal;
...
"#).unwrap();
    assert!(
        matches!(&events2[0].signal, RuntimeSignal::UsageUpdated { usage } if usage.output == 15)
    );
}
...
"#,
    ...
    let events = p2
        ...
        .unwrap();
    assert!(
        matches!(&events[2].signal, RuntimeSignal::TranscriptBlockDelta { index: 0, delta } if delta == "Hi")
    );
}
...
fn invalid_json_produces_error_event_and_tool_use_streams_partial_json() {
    ...
    let events = parser
        ...
        .unwrap();
    assert!(
        matches!(&events[0].signal, RuntimeSignal::Error { code, .. } if code == "sse_parse_error")
    );
    ...
    assert!(
        matches!(&start[1].signal, RuntimeSignal::TranscriptBlockStart { index: 1, block: StreamBlock::ToolCall { id, name, .. } } if id == "toolu_123" && name == "write_file")
    );
    ...
    assert!(
        matches!(&delta[1].signal, RuntimeSignal::ToolCallArgumentsDelta { delta, .. } if delta == "{\"path\":\"src/")
    );
}
...
"#).unwrap();
    assert!(
        matches!(&events1[3].signal, RuntimeSignal::TranscriptBlockDelta { delta, .. } if delta == "Reading file now. ")
    );
...
"#).unwrap();
    assert!(events2.iter().any(|e| matches!(&e.signal, RuntimeSignal::ToolCallStarted { tool_name, .. } if tool_name == "read_file")));
...
"#).unwrap();
    assert!(events3.iter().any(
        |e| matches!(&e.signal, RuntimeSignal::TranscriptBlockDelta { delta, .. } if delta == "World")
    ));
}
...
fn sse_protocol_id_retry_and_edge_case_fields() {
    ...
    assert_eq!(parser.last_frame_id(), Some("evt-42"));
    parser.process(b"id: second\ndata: {}\n\n").unwrap();
    assert_eq!(parser.last_frame_id(), Some("second"));
