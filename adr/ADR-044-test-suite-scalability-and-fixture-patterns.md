...
- Collapse repeated scenarios into `#[case]` parameterized tests via [`rstest`](https://docs.rs/rstest).
  `rstest` is preferred over `test-case` because it supports async tests natively (composing with
  `#[tokio::test]`), handles complex fixture types, and is already present in `Cargo.toml`.
...
- [`rstest`](https://docs.rs/rstest) — parameterized test macros with async + fixture support
