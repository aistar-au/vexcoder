...
| `adr/ADR-039-neutral-cli-voice-and-spatial-status-language.md` | Proposed | 4 batches (A-D): low-gain vocabulary and status rollout first, then ANSI roles and paragraph progress stream |
...
That rollout starts with subtle wording changes on existing surfaces before it
introduces the semantic violet status lane or the denser paragraph stream. The
...
| 11 | ADR-039 | 4 | Proposed | Subtle vocabulary pass, status copy anchors, ANSI roles, paragraph progress stream |
...
proposed lane, starting with low-gain wording normalization before later ANSI
and transcript-model changes. The only deferred external follow-up is PG-03
