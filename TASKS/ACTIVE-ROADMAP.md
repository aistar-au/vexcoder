...
| :--- | :--- | :--- | :--- | :--- |
| REF-01 follow-up | `work/vexcoder-runtime-envelope-client-api-direct-consumption` | #403 | **In Progress** | Whole-system continuation after PR #402 is complete in this branch: residual client/API-side `StreamEvent` and `ContentBlock` parsing now reaches direct `RuntimeEnvelope` consumption where the layer is an internal API consumer, the CLI and ratatui/crossterm stack remain downstream API consumers, and later branches stage structural extraction for `src/runtime/json_handoff.rs` and `src/api/stream.rs` |
