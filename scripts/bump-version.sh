...
echo "  3. After merge on main, tag locally: git tag -a v$NEW_VERSION -m \"Release v$NEW_VERSION\""
echo "  4. Push the tag: git push origin v$NEW_VERSION"
echo "  5. Do not open a separate PR patch for the tag; the local tag push triggers the release workflow"
