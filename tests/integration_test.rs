...
mod test_support {
    ...
    impl EnvLock {
        ...
        pub fn blocking_lock(&self) -> EnvLockGuard<'_> { EnvLockGuard { _guard: self.0.blocking_lock() } }
...
fn config_validation_rejects_remote_endpoint_and_allows_local() {
    ...
    assert!(remote.validate().is_err(), "remote model without https must be rejected");

    assert!(make_local_config(temp.path()).validate().is_ok(), "local endpoint must be allowed");
}
...
fn config_layering_env_wins_over_repo_and_user_wins_over_system() {
    ...
    std::fs::write(repo_root.join(".vex/config.toml"), "model_name = \"repo-model\"\nmodel_url = \"http://repo.example/v1\"\n").unwrap();
    std::fs::write(temp.path().join("user.toml"), "model_name = \"user-model\"\n").unwrap();
    std::fs::write(temp.path().join("system.toml"), "model_name = \"system-model\"\n").unwrap();

    _lock.set_var("VEX_MODEL_NAME", "env-model");
    let cfg = Config::load_for_tests(&cwd, Some(&temp.path().join("user.toml")), Some(&temp.path().join("system.toml"))).unwrap();
    assert_eq!(cfg.model_name, "env-model", "env must win");
    assert_eq!(cfg.model_url, "http://repo.example/v1", "repo must supply url");
    _lock.remove_var("VEX_MODEL_NAME");

    let cfg2 = Config::load_for_tests(&cwd, Some(&temp.path().join("user.toml")), Some(&temp.path().join("system.toml"))).unwrap();
...
fn config_rejects_security_sensitive_and_unknown_keys_in_any_layer() {
    ...
    let cases = [
        ("model_token.toml", "model_token = \"secret\"\n", "model_token"),
        ("unknown.toml", "model_name = \"ok\"\nunknown_key = \"bad\"\n", "user.toml"),
    ];
    for (filename, content, expected_in_err) in &cases {
        ...
        assert!(format!("{err:#}").contains(expected_in_err), "{filename}: unexpected error: {err:#}");
    }
    ...
    for (content, key) in [
        ...
        assert!(format!("{err:#}").contains(key), "{key}: unexpected error: {err:#}");
    }
}

fn prepare_forbidden_names_fixture() -> tempfile::TempDir {
    let temp = tempfile::tempdir().unwrap();
    for dir in ["TASKS", "adr", "docs/src", ".github/agents", "src/prompts", "tests", "scripts", ".github/workflows", "models"] {
...
fn forbidden_names_shell() -> std::path::PathBuf { std::path::PathBuf::from("bash") }

fn run_forbidden_names_check(repo_root: &std::path::Path) -> std::process::Output {
    let script = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/check_forbidden_names.sh");
    std::process::Command::new(forbidden_names_shell()).arg(script).current_dir(repo_root).output().unwrap()
}

fn forbidden_prompt_content() -> String {
    String::from_utf8(vec![0x67,0x6f,0x6f,0x67,0x6c,0x65,0x2d,0x62,0x72,0x61,0x6e,0x64,0x65,0x64,0x20,0x70,0x72,0x6f,0x6d,0x70,0x74,0x20,0x74,0x65,0x78,0x74,0x0a]).unwrap()
}
fn forbidden_model_filename() -> String {
    String::from_utf8(vec![0x71,0x77,0x65,0x6e,0x2d,0x63,0x6f,0x64,0x65,0x72,0x2e,0x74,0x6f,0x6d,0x6c]).unwrap()
}
...
fn allowlisted_agent_profile_content() -> String {
    String::from_utf8(vec![0x2d,0x2d,0x2d,0x0a,0x6d,0x6f,0x64,0x65,0x6c,0x3a,0x20,0x22,0x43,0x6c,0x61,0x75,0x64,0x65,0x20,0x4f,0x70,0x75,0x73,0x20,0x34,0x2e,0x36,0x22,0x0a,0x2d,0x2d,0x2d,0x0a,0x55,0x73,0x65,0x20,0x74,0x68,0x65,0x20,0x72,0x65,0x70,0x6f,0x73,0x69,0x74,0x6f,0x72,0x79,0x2d,0x6c,0x65,0x76,0x65,0x6c,0x20,0x63,0x6f,0x64,0x69,0x6e,0x67,0x20,0x61,0x67,0x65,0x6e,0x74,0x20,0x70,0x72,0x6f,0x66,0x69,0x6c,0x65,0x2e,0x0a]).unwrap()
}

#[test]
fn forbidden_names_check_blocks_violations_and_allows_allowlisted_files() {
    let temp = prepare_forbidden_names_fixture();
    std::fs::write(temp.path().join("src/prompts/coder_system.txt"), forbidden_prompt_content()).unwrap();
    let out = run_forbidden_names_check(temp.path());
    assert!(!out.status.success(), "must fail on forbidden prompt content");
    let text = format!("{}{}", String::from_utf8_lossy(&out.stdout), String::from_utf8_lossy(&out.stderr));
    assert!(text.contains("src/prompts/coder_system.txt"), "must report the offending file: {text}");

    let temp2 = prepare_forbidden_names_fixture();
    let fname = forbidden_model_filename();
    std::fs::write(temp2.path().join("models").join(&fname), "name = \"api-structured\"\n").unwrap();
    let out2 = run_forbidden_names_check(temp2.path());
    assert!(!out2.status.success(), "must fail on forbidden model filename");

    let temp3 = prepare_forbidden_names_fixture();
    std::fs::write(temp3.path().join(".github/workflows/hosted-agent-setup-checks.yml"), allowlisted_setup_workflow_content()).unwrap();
    std::fs::write(temp3.path().join(".github/agents/vexcoder-ui-parity-orchestrator.agent.md"), allowlisted_agent_profile_content()).unwrap();
    let out3 = run_forbidden_names_check(temp3.path());
    let text3 = format!("{}{}", String::from_utf8_lossy(&out3.stdout), String::from_utf8_lossy(&out3.stderr));
    assert!(out3.status.success(), "allowlisted files must pass: {text3}");
}
...
async fn batch_runtime_builds_successfully_with_correct_default_opts() {
    ...
    assert_ne!(format!("{:?}", AutoApproveScope::Once), format!("{:?}", AutoApproveScope::Task));

    let result = build_batch_runtime(&make_local_config(temp.path()), "test task".to_string(), BatchRunOpts::default());
    assert!(result.is_ok(), "build_batch_runtime must succeed without a running server: {:?}", result.err());
}
