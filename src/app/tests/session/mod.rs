...
fn new_command_saves_state_resets_transcript_and_creates_fresh_id() {
    ...
    assert!(temp.path().join(format!("{original_id}.json")).exists(), "/new must save prior task");
    assert_ne!(mode.current_task_id(), original_id, "/new must create fresh task-id");
...
fn resume_restores_active_grants_and_rejects_unknown_id() {
    ...
    state.active_grants.insert("read-file".to_string(), crate::runtime::ApprovalScope::Session);
    ...
    assert!(mode.task_doc.info.active_grants.contains_key("read-file"));

    mode.on_user_input("/resume no-such-task-99".to_string(), &mut ctx);
    assert!(mode.history_lines().iter().any(|l| l.contains("not found") || l.contains("error") || l.contains("unknown")),
        "unknown id must emit error; lines: {:?}", mode.history_lines());
