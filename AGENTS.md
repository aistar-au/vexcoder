...
| `adr/ADR-031-operator-surface-ui-overhaul.md` | Operator-surface timeline overhaul, merge-gated UI batching, and task-state-first adaptive timeline/transcript/composer rules. |
...
| ADR-031 | Active | Defines the timeline-driven operator surface overhaul, merge-gated UI batch ordering, and task-state-first adaptive task-surface behavior layered on ADR-030. Batch A/B/C implemented: persistent four-region layout, last-turn data preservation, enriched paragraph rendering, adaptive layout engine, human-readable header with cumulative context indicator, flowing transcript styling with Unicode markers, and inline approval cards in the composer. |
