# ADR-050: Doc-Codepath and Context Drift Detection
...
A line-by-line audit of merged PRs #428, #429, #431, #433, #434 against both manifests yields the following invariants and drift cases.

### Observed drift cases

1. **Renamed module not propagated.** `src/api/eventsource{,/non_stream,/tests,/tests/local_retry,/tests/stream_lifecycle}.rs` (5 paths) appear in `REPO-RAW-URL-MAP.md` but no longer exist on disk. Their replacements `src/api/stream_ingress*` (5 paths) exist but are absent from the map. Path-set cardinality is unchanged, so the count-parity check in `doc-ref-check.yml` passes while the path-set diff is non-empty.
...
5. **Recent ADR not indexed.** `adr/ADR-049-shared-prefix-prompt-caching-and-fork-controls.md` exists on `main` and is not in `REPO-RAW-URL-MAP.md`. Header count remains 417 because an unrelated path was removed in the same window, again masking the drift under count parity.

### Context-budget observation

The local PR-435 sketch in `../memory.md` identifies a second invariant: recurring operator notes must stay in a disk-backed memory file and enter the model prompt only through a bounded injection path. The repository implementation maps that requirement to `src/session_notes.rs`, `src/api/client/mod.rs`, and `src/config/load/`: `notes_path` is rejected in repo-local config, the default notes path is XDG-style `memory.md`, and `max_memory_tokens` prevents unbounded notes content from entering the system prompt. This mirrors the external pattern observed in comparable coding-agent surfaces: persistent Markdown guidance is file-backed, scoped by repository or path, and loaded into context according to precedence and budget rules rather than copied into every prompt manually.

### Root cause

The repository's current invariant is path-set count, not path-set membership. A rename that preserves cardinality, or a removal balanced by an unrelated addition, avoids the check. Documentation drift detection literature treats this exact pattern as a count-versus-content mismatch and recommends diff-driven membership comparison rather than scalar coverage [DDD-1, ARCH-1].
...
A count-parity check is a necessary but not sufficient invariant. Two operations that preserve cardinality (rename, balanced add/remove) leave the manifest semantically wrong while leaving the workflow's signal passing. The drift cases above are evidence: each was authored under passing CI while still producing an obsolete agent path.

A membership check has the property that any symbol-bearing rename is forced to update the manifest by definition, because the symmetric difference is non-zero until the row moves. Membership is the smallest invariant that catches all observed drift cases without introducing semantic understanding of file contents.

The choice of `REPO-RAW-URL-MAP.md` as canonical (over the inline `CONTRIBUTING.md` table) follows from the workflow's anchor: the workflow already validates that file. Two manifests with overlapping ownership and one CI anchor are a weaker invariant than one manifest with the same anchor.
...
- This PR updates `REPO-RAW-URL-MAP.md` for the five `stream_ingress` paths and ADR-049, and removes the replaced `eventsource` rows from the canonical manifest.
- A subsequent PR MUST land the membership check in `doc-ref-check.yml`. It SHOULD also reduce or remove the inline table from `CONTRIBUTING.md` so there is one canonical path manifest.
...
- The api-types sub-crate row noted in §"Observed drift cases" claim 2 is already present in the canonical manifest under its actual path `crates/vexcoder-api-types/src/lib.rs`; no further action is required for that row in this PR.
...
- **R2 — Span-aware wrapping**. Anchor: `src/ui/render/transcript.rs` `word_wrap_transcript_row` and `word_wrap_plain_row`. Current behaviour: early return when `contains_ansi_escape(text)` is true. Target behaviour: flatten the styled `Line` into a string while recording per-span byte ranges, wrap under a `WordSplitter` that does not split CSI sequences, then reconstruct styled spans from the recorded ranges. Comparable Rust TUI implementations use this flatten-record-wrap-reconstruct structure for styled text. Adopt the structural approach, not any verbatim symbols.
