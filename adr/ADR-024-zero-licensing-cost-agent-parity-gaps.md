...
| IDE extensions | Deferred to a post-phase-1 ADR per ADR-022 amendment Decision item 11. File-based editor extensions must use `vex exec` (Gap 2). Native GUI surfaces (IDE panels with live streaming, macOS native client) must use the `LocalApiServer` path reserved in Phase I |
...
**Phases G and H (distribution and macOS packaging) are post-phase-1** and must not block phase-1 correctness work (ADR-022 phases 1–8 and ADR-023 edit loop). They may not begin until the edit loop, approval system, and task state persistence are validated end-to-end. Any implementation lane that begins Phase G or H work before those gates are green must be considered out of scope.

**Phase I (local API server)** requires a dedicated ADR specifying wire protocol, local socket authentication, and streaming response format before any implementation lane begins work. It may not begin before phase-1 correctness work is validated end-to-end.
...
**Scope boundary:** These are thin wrappers over `git` binary calls and `ContextAssembler`-style diff assembly. They do not integrate with any specific hosting platform API. Platform API integration (creating PRs via REST API) is explicitly out of scope and requires a dedicated ADR. `vex pr-summary` produces a text artifact the operator pipes to whatever CLI tool manages their remote.
...
Reference hosted agent CLIs display per-turn and cumulative token usage from API responses. This gap is explicitly separable from the compaction deferral gate: counting tokens reported in API responses requires no summarisation strategy and must not be blocked by the compaction ADR requirement.
...
| Release artifact | `.dmg` attached to hosted release entry |
...
- Operators can install `vex` from hosted release artifacts without building from source.
...
- Phases G and H must not begin until phase-1 correctness work is validated end-to-end.
...
| Do not begin Phases G or H before phase-1 correctness work is validated | Sequencing guard |
...
The current command-execution amendment is recorded in `adr/ADR-022-amendment-2026-03-13.md` and must be applied to `adr/ADR-022-free-open-coding-agent-roadmap.md` before Phases G-H work begins. See that file for exact application instructions.
...
- `adr/ADR-022-amendment-2026-03-13.md` — command-execution migration notes and current runtime contract
...
- Notes:
  - `vex completions`, `vex install-hooks` / `vex uninstall-hooks`, and `vex skills` were implemented on this operator branch and now match the ADR-024 surface contract.
