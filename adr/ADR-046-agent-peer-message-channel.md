...
- Per-`session_task_id` append-only JSONL channel at `.vex/state/{session_task_id}.channel.jsonl`.
- Any session task sharing that `session_task_id` may post to and read from the channel.
...
- Message format: JSON object with `id`, `sent_at` (pagination cursor), `sender_id`, `sender_agent_id`, `recipient`, `kind`, `content`, `session_task_id`.
...
- Reserve `PeerMessagePosted` `RuntimeSignal` variant for ADR-045 compatibility.
