...
async fn schedule_team_handler_returns_session_task_ids() {
    ...
    let response = router.oneshot(
        Request::builder()
            ...
            .body(Body::from(r#"{"parent_task_id":"sched-parent","prompt":"review the code"}"#))
            .unwrap(),
    ).await.unwrap();
    ...
    let ids = payload.get("session_task_ids").and_then(Value::as_array).expect("session_task_ids");
    assert!(!ids.is_empty());
}
...
async fn schedule_team_returns_not_found_for_unknown_team_and_rejects_blank_prompt() {
    ...
    let not_found = router.clone().oneshot(
        Request::builder()
            .method("POST")
