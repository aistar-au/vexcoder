...
async fn fallback_writes_protocol_discovery_and_shape_debug_records_for_both_protocols() {
    for (label, spawn_fn, expected_protocol, expected_count_key) in [
        ("chat-compat", "chat", "chat-compat", "\"tool_argument_object_count\":1"),
        ("messages-v1", "messages", "messages-v1", "\"tool_use_count\":1"),
    ] {
        ...
        let (base_url, server) = if spawn_fn == "chat" { spawn_tool_calls_json_args_server().await } else { spawn_auto_detect_messages_v1_tool_calls_server().await };
        let model_name = if spawn_fn == "chat" { "json-args-model" } else { "tool-detect-model" };
        let prompt = if spawn_fn == "chat" { "Read main.rs." } else { "Read lib.rs." };
        ...
        while let Some(envelope) = stream.next().await { envelope.expect("envelope must not error"); }
        ...
        for expected in [
            ...
            assert!(log.contains(expected), "{label}: missing {expected} in log: {log}");
        }
        server.abort();
    }
}
