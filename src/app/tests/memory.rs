...
fn memory_command_renders_empty_notes_without_pulse() {
    ...
    assert!(mode.history_lines().iter().any(|l| l.contains("[memory] no notes")));
...
fn memory_add_appends_to_file_and_rejects_clear_without_confirmation() {
    ...
    mode.on_user_input("/memory add track the open build issue".to_string(), &mut ctx);
    ...
    assert!(mode.history_lines().iter().any(|l| l.contains("[memory] confirm")),
...
fn build_runtime_auto_index_warms_codebase_search_index() {
    ...
    let search_config = crate::config::SearchConfig { enabled: true, auto_index: true, ..Default::default() };
