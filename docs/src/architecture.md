...
O(total_content) — and expose pending deltas for the render layer. Delta
...
`#[allow(unused)]` annotations until the live render path switchover activates
them; the remaining foundation methods are now used by the active code paths.
The ratatui render module and `format_compact_paragraph()` provide the
display path that routes transcript content through the `[tool]`/`[detail]`/
`[evidence]` prefix-marker chain (ADR-041 D5–D7).
