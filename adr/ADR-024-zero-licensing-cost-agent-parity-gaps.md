...
| Repo-local restriction | `[[mcp_servers]]` in repo-local config rejected with diagnostic |
...
| Do not begin Phases G or H before initial correctness work is validated | Sequencing guard |
...
| `-p`/`--print` must not be implemented before Gap 2 (`BatchMode`) is complete | `--print` is a routing option over `BatchMode`; implementing it without `BatchMode` requires duplicating runtime logic, which is not allowed |
