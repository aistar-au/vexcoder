...
pub(crate) fn missing_read_only_location_prompt(
    ...
    match name {
        "read_file" => {
            if missing(&["path", "file_path", "file", "filename"]) {
                Some("I need an explicit file path before reading a file. Please provide a non-empty `path` such as `src/main.rs` or `adr/ADR-README.md`. If the user referenced a file with `@`, a `[file: ...]` block may already be in the conversation. Reuse that embedded content when it is sufficient; otherwise extract the explicit path from the block and call `read_file` or `search_files` for targeted inspection. Do not retry without a concrete path. No file changes were made.".to_string())
            } else {
                None
            }
        }
