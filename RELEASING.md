...
```
0.1.0-alpha.1 -> 0.1.0-alpha.2 -> ... -> 0.1.0-beta.1 -> 0.1.0-beta.2 -> 0.1.0-beta.3 -> 0.1.0-beta.4 -> 0.1.0-beta.8 -> 0.1.0-rc.1 -> 0.1.0
```
...
`0.1.0-alpha.2 < 0.1.0-alpha.3 < 0.1.0-beta.1 < 0.1.0-beta.2 < 0.1.0-beta.3 < 0.1.0-beta.4 < 0.1.0-beta.8 < 0.1.0-rc.1 < 0.1.0`.
...
- Tag names must match the `Cargo.toml` version exactly (with the `v` prefix):
  if `Cargo.toml` says `0.1.0-beta.8`, the tag is `v0.1.0-beta.8`.
...
git tag -a v0.1.0-beta.8 -m "Release v0.1.0-beta.8"
...
10. Verify the tag exists on the remote:
    ```bash
  git ls-remote --tags origin | grep v0.1.0-beta.8
    ```
...
12. Verify the workflow published the release entry, attached the platform
  ...
  `CHANGELOG-v0.1.0-beta.8.md` asset.
...
2. Enter the new version (e.g. `0.1.0-beta.8`). No `v` prefix.
