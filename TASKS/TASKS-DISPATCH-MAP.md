...
- Treat ADR-031 and ADR-032 as merged operator-surface prerequisites that downstream retrieval work must preserve.
...
- The useful remaining follow-up batch is prompt/documentation alignment for those merged ADR-033 contracts; the old ADR-031 Batch A, ADR-032 prompt-interactivity, and ADR-033 Phase 3/4 review branches no longer differ from current `main`.
