...
- ADR-034 defines the proposed post-launch multi-agent lane: worktree-isolated agent definitions, orchestrator-owned session-task lifecycle, `/agents`, `/watch`, and explicit session-task release surfaces, plus delegation-time concurrency and prompt-size enforcement built on the canonical ADR-025/ADR-030 contracts. The current hardening pass makes the delegation cap serialized, adds release-route and concurrency-stress coverage, and normalizes parent-task watch rollups onto the same lowercase status surface used by session tasks.
