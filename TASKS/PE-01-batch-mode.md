# Task PE-01: Non-interactive Execution Mode (BatchMode)
...
5. JSONL output (one object per turn, written after each turn completes):
   ...
   ```json
   {
     ...
     "tokens": {"input": 512, "output": 128, "estimated": false}
   }
   ```
...
- All six anchor tests below pass.
...
async fn test_batch_mode_max_turns_stops_run() {
    // A task that would run indefinitely if unconstrained; max_turns = 1 must
    // stop it and produce MaxTurnsReached status.
...
async fn test_batch_mode_jsonl_output_includes_required_fields() {
    ...
    // Each turn line must have at minimum: turn, response, changed_files,
    // command_history.
