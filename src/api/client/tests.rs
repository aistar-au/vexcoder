...
async fn client_falls_back_to_non_streaming_chat_compat_when_stream_is_slow() {
    ...
    let (server, addr) = spawn_axum_server(
        Router::new().route("/v1/chat/completions", post(handler)).with_state(requests.clone()),
    ).await;
    let config = local_stream_test_config(format!("http://{addr}/v1/chat/completions"), ModelProtocol::ChatCompat);
    let client = ApiClient::new(&config).expect("client");
    let mut stream = client.create_stream(&single_user_message("Reply OK.")).await.expect("stream");
    let envelopes: Vec<_> = futures::StreamExt::collect::<Vec<_>>(&mut stream).await
...
async fn client_falls_back_to_non_streaming_messages_v1_when_stream_is_slow() {
    ...
    let requests: RequestLog = Arc::new(Mutex::new(Vec::new()));
    let (server, addr) = spawn_axum_server(
        Router::new().route("/v1/messages", post(handler)).with_state(requests.clone()),
    ).await;
    let config = local_stream_test_config(format!("http://{addr}/v1/messages"), ModelProtocol::MessagesV1);
    let client = ApiClient::new(&config).expect("client");
    let mut stream = client.create_stream(&single_user_message("Read src/lib.rs")).await.expect("stream");
    let envelopes: Vec<_> = futures::StreamExt::collect::<Vec<_>>(&mut stream).await
        .into_iter().filter_map(|r| r.ok()).collect();
    server.abort();
    let reqs = requests.lock().unwrap();
...
fn map_api_status_error_encodes_protocol_hint_and_server_message() {
    let msg = format!("{}", map_api_status_error(StatusCode::BAD_REQUEST, "invalid model name", "http://localhost:8000/v1/messages", None));
    assert!(msg.contains("MessagesV1"), "400: got: {msg}");
    let msg = format!("{}", map_api_status_error(StatusCode::INTERNAL_SERVER_ERROR, "out of memory", "http://localhost:8000/v1/messages", None));
    assert!(msg.contains("500") && msg.contains("out of memory"), "500: got: {msg}");
}
...
async fn map_api_request_error_local_connect_failure_mentions_retry() {
    ...
    let err = reqwest::Client::new().post(&url).send().await.expect_err("should fail");
    let msg = map_api_request_error(err, &url).to_string();
    assert!(msg.contains("retries short-lived local startup connection failures"), "got: {msg}");
}
