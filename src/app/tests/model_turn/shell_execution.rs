...
async fn bang_prefix_runs_shell_without_model_turn_after_approval() {
    ...
    assert!(
        mode.history_lines()
            .iter()
            .any(|l| l.contains("[command session started"))
    );
    assert!(
        mode.history_lines()
            .iter()
            .any(|l| l.contains("inline-shell"))
    );
    assert!(
        mode.history_lines()
            .iter()
            .any(|l| l == "[command session exit: 0]")
    );
}
...
async fn shell_command_runner_invokes_sandbox_wrap() {
    ...
    assert!(
        ...
        "sandbox must have been invoked"
    );
